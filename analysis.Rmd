...
```{r type}
ggplot(academic, aes(x=audit_type_condensed)) +
  geom_bar() +
  coord_flip()
...
academic_formatted = academic %>%
  group_by(audit_type_condensed) %>%
  arrange(audit_type_condensed) %>% # preserve factor ordering
  mutate(
    audit_type_condensed = paste0(as.character(audit_type_condensed), "<br>*(N=", n(), ")*"),
    audit_type_condensed = factor(audit_type_condensed, unique(audit_type_condensed)),
    # audit_type_condensed = str_wrap_factor(
    #   audit_type_condensed,
    #   width=14, whitespace_only=F
    # )
  )

plot_bar(academic_formatted, "year") + xlab("Year published")
ggsave("plots/bar_time_v2.pdf", dpi=300, width=5, height=4)

plot_bar(academic_formatted, "source_condensed") + xlab("Publication venue")
ggsave("plots/bar_source_v2.pdf", dpi=300, width=5, height=4)
```
